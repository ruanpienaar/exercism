...
data_is_retained_test() ->
    Exp = t1(),
    ?assertEqual(Exp, zipper:to_tree(zipper:from_tree(t1()))).

left_right_and_value_test() ->
    ?assertEqual(3, zipper:value(zipper:right(zipper:left(zipper:from_tree(t1()))))).

dead_end_test() ->
    Exp = empty(),
    ?assertEqual(Exp, zipper:left(zipper:left(zipper:from_tree(t1())))).

tree_from_deep_focus_test() ->
    Exp = t1(),
    Zipper = zipper:right(zipper:left(zipper:from_tree(t1()))),
    ?assertEqual(Exp, zipper:to_tree(Zipper)).

traversing_up_from_top_test() ->
    Exp = empty(),
    ?assertEqual(Exp, zipper:up(zipper:from_tree(t1()))).
...
left_right_and_up_test() ->
    ...
    ?assertEqual(3, zipper:value(Act6)).
...
set_value_from_deep_focus_test() ->
    ...
    ?assertEqual(Exp, Act4).

version_test() ->
  ?assertEqual(1, zipper:test_version()).
